...
function BuildOrder(props) {
    ...
    if (barcodeScan) {
        return (
            <BarCodeScan barCodeData={handleBarCodeData} />
        )
    }
    return (
        <View style={{ flex: 1, paddingHorizontal: 24, backgroundColor: colors.white }}>
            ...
            <View style={[commonStyles.searchContainer, { marginTop: 12, marginBottom: 12 }]}>
                ...
                {searchText !== '' ? <TouchableOpacity onPress={() => searchProduct('')}
                    style={{ position: "absolute", right: 28, padding: 10 }}>
                    <AntDesign name="close" size={18} color={colors.grey} />
                </TouchableOpacity> : null}

                <TouchableOpacity onPress={() => setBarcodeScan(true)} style={{ position: "absolute", right: 0, padding: 10 }}>
                    <Icons name="barcode-scan" size={22} color={colors.grey} />
                </TouchableOpacity>
            </View>
            <FlatList
                data={productsData}
