        ...
        <View>
            <View style={styles.underline}>
                {props.item.product_group_id ? (
                    <Text style={texts.darkGreyTextBold14}>{props.item.company_name}</Text>
                ) : (
            ...
            <View style={[commonStyles.rowSpaceBetween, {width: "100%"}]}>
                <View style={{width: "86%"}}>
                    {props.item.product_group_id ? <TouchableOpacity onPress={() => {
                        props.expandProductGroupImage(props.index)
                    }} style={[commonStyles.rowAlignCenter, {paddingVertical: 6}]}>
                        {!props.item.pg_image_expanded ? <View style={{marginRight: 6}}>
                            <Image style={styles.productImage}
                                   source={props.item.image ? {uri: props.item.image} : require('../../assets/images/placeholder_profile_pic.jpg')}/>
                        </View> : null}
                        <View style={{width: "86%"}}>
                                            ...
                                            {entity.unit_conversion && (entity.level_1_enabled || entity.level_2_enabled) ?
                                                <View style={{width: 80}}>
                                                    <Dropdown
                                                        ...
                                                        onChange={unit => {
                                                            props.selectUnitDropdown(props.index, subIndex, unit.value, unit.quantity, unit.label)
                                                        }}
                                                    />
                                                </View> : null}
                                        </View>
