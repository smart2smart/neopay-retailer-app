...
export default function ProfileScreen(props) {
  ...
  const goToMapView = () => {
    let currentLocation = {
      latitude: retailerData.address_data.latitude,
      longitude: retailerData.address_data.longitude,
    };
    navigation.navigate("MapViewScreen", {
      comingFrom: "profile",
      addressId: retailerData.address_data.id,
      ...
      >
        <View>
          {retailerData.attachment ? (
            <View style={commonStyles.imageContainer}>
              <Image
                source={{ uri: retailerData.attachment }}
                style={{ width: "100%", height: "100%" }}
              />
              <TouchableOpacity
                onPress={goToUploadImage}
                style={style.editButtonDiv}
              >
        ...
        >
          <View style={style.textContainer}>
            <Text style={[texts.blackTextBold18]}>{retailerData.name}</Text>
            ...
            <View style={commonStyles.rowSpaceBetween}>
              {retailerData.address_data &&
              retailerData.address_data.latitude != 0 ? (
                <View>
                  <View>
                    <Text style={texts.greyTextBold14}>
                      {retailerData.address_data.latitude
                        .toString()
                        .substring(0, 10)}
                      ,{" "}
                    </Text>
                    <Text style={texts.greyTextBold14}>
                      {retailerData.address_data.longitude
                        .toString()
                        .substring(0, 10)}
                    </Text>
                  </View>
                  <TouchableOpacity onPress={goToMapView}>
                    <Text
                      style={[
                        texts.redTextBold14,
                        { textDecorationLine: "underline", paddingTop: 5 },
                      ]}
                    >
                      Change
                    </Text>
        ...
        >
          ...
          <Image
            ...
            source={require("../../assets/images/Group_582.png")}
          />
        </TouchableOpacity>
        <TouchableOpacity
          style={[style.textContainerWrapper, commonStyles.rowSpaceBetween]}
        ...
        <TouchableOpacity
          style={[
            style.textContainerWrapper,
            commonStyles.rowSpaceBetween,
            { marginBottom: 20 },
          ]}
        >
          <Text style={[texts.blackTextBold14, { opacity: 0.2 }]}>
            Payments
          </Text>
          <Image
            style={style.phoneIcon}
