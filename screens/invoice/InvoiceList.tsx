  ...
  const searchInvoice = (text: string) => {
    if (text !== "") {
      let data = originalInvoiceData.filter((item) => {
        return (
          item.invoice_original_value.toString().includes(text.toLowerCase()) ||
  ...
  return (
    <View style={{ flex: 1, paddingHorizontal: 24 }}>
      ...
      <View style={[commonStyles.rowSpaceBetween, { marginTop: 12 }]}>
        <Text style={texts.greyTextBold14}>Invoices Till:</Text>
        <TouchableOpacity
      ...
      {invoiceData.length === 0 && !isLoading ? (
        ...
        >
          <Text style={texts.darkGreyTextBold16}>No invoices available</Text>
        </View>
      ) : (
