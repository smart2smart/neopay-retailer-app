...
function OrderList(props) {
  ...
  const getOrderData = (paginate) => {
    ...
    AuthenticatedGetRequest(data).then((res) => {
      ...
      setOrdersData(
        paginate ? [...ordersData, ...res.data.results] : res.data.results
      );
      setOriginalOrdersData(
        paginate ? [...ordersData, ...res.data.results] : res.data.results
      );
    });
  };
    ...
    <View style={{ flex: 1 }}>
      ...
      <View style={{ paddingHorizontal: 12, flex: 1 }}>
        ...
        <View style={[commonStyles.rowSpaceBetween, { paddingTop: 16 }]}>
          <Text style={texts.greyTextBold14}>Orders Till:</Text>
        </View>
        {ordersData.length === 0 && !isLoading ? (
          <View style={[commonStyles.rowCenter, { flex: 1 }]}>
            <Text style={texts.darkGreyTextBold16}>No orders available</Text>
          </View>
        ) : (
