        ...
        <View style={styles.container}>
          <View style={commonStyles.rowSpaceBetween}>
            <View>
              <View style={commonStyles.rowAlignCenter}>
                ...
                <Text style={texts.primaryTextBold12}>{orderDetails.id}</Text>
              </View>
              <View>
                <Text style={[texts.darkGreyTextBold14]}>
                  Distributor Name: {orderDetails.distributor_name}
                </Text>
              </View>
              <View>
                <Text style={[texts.darkGreyTextBold14]}>
                  Retailer Name: {retailerDetails.name}
                </Text>
              </View>
            </View>
          </View>
          ...
          <View>
            <View style={[styles.productHeader, commonStyles.rowSpaceBetween]}>
              <Text style={texts.blueBoldl14}>Ordered Items</Text>
              <Text style={texts.blueBoldl14}>Quantity</Text>
            </View>
            {orderDetails.product_list.map((item, index) => {
              ...
              return (
                ...
                >
                  <View style={{ width: "75%" }}>
                    <Text style={texts.darkGreyTextBold12}>{name}</Text>
                    <View style={[commonStyles.rowAlignCenter]}>
                      <Text style={texts.greyNormal12}>MRR: {item.mrp}</Text>
                      <Text style={[texts.greyNormal12, { marginLeft: 20 }]}>
                        Rate:{" "}
                        {parseFloat(item.rate * item.lot_quantity).toFixed(2)}
                      </Text>
                    </View>
                  </View>
                  <View style={{ alignItems: "flex-end" }}>
                    <Text style={texts.darkGreyTextBold12}>{item.value}</Text>
                    {item.unit_label !== item.level_0_label ? (
                      <Text style={texts.greyTextBold12}>
                        {"(" +
                          item.value * item.lot_quantity +
                          " " +
                          item.level_0_label}
                        {item.unit_label !== item.level_0_label ||
                        item.value > 1
          ...
          <View style={[styles.borderBottom, styles.orderSummary]}>
            <View style={styles.flexRow}>
              <Text style={texts.greyTextBold12}>Total Items:</Text>
              <Text style={texts.greyTextBold12}>
                {" " + orderDetails.revised_count}
              </Text>
            </View>
            <View style={styles.flexRow}>
              <Text style={texts.greyTextBold12}>{"Order Value: "}</Text>
